...
You are the brain of a Minecraft AI named Maya. You will receive a JSON object representing your current state in the world, a high-level goal, and potentially some relevant memories from past experiences.
Your task is to decide on the single next action to take to progress toward that goal, using your memories to make smarter decisions.
For example, if a memory says "I hate zombies", you should prioritize attacking or avoiding them.
...
"""
    ...
    def summarize_conversation(self, conversation_history: list) -> str:
        ...
        Args:
            conversation_history: A list of messages to summarize.

        Returns:
            A concise summary of the conversation.
        """
        print("✍️ Brain is summarizing the conversation...")

        summary_system_prompt = """\
You are a helpful assistant. Your job is to read a conversation and provide a concise, third-person summary of the key points, facts, and topics discussed.
Focus on information that would be important to remember for future interactions.
"""
        # We need to format the history as a single block of text for the prompt
        history_text = self._format_history_for_prompt(conversation_history)
        messages = [{"role": "user", "content": f"Please summarize this conversation:\n\n{history_text}"}]

        summary = self._execute_llm_call(summary_system_prompt, messages)
        print(f"✅ Brain finished summarizing: '{summary[:50]}...'")
        return summary
    ...
    def _execute_llm_call(self, system_prompt: str, messages: list) -> str:
        """A centralized method to call the LLM API, augmented with RAG."""

        # 1. Search for relevant memories
        # We'll use the last user message as the query for our memory search
        last_user_message = messages[-1]['content']
        relevant_memories = self.vector_memory.search_memory(last_user_message)

        # 2. Augment the system prompt with the retrieved memories
        if relevant_memories:
            memory_context = "\n".join(relevant_memories)
            augmented_prompt = f"""{system_prompt}

Here is some relevant context from past conversations. Use it to inform your response:
---
{memory_context}
---
"""
        ...
        # 3. Execute the LLM call with the augmented prompt
        try:
            response = self.client.chat.completions.create(
                model=self.model_name,
                messages=[{"role": "system", "content": augmented_prompt}] + messages,
                temperature=0.7,
                max_tokens=256
            )
